...
_MESSAGES: dict[str, dict[str, str]] = {
    "en": {
        ...
        "tip_context_efficiency_specific": "{names} are high context-cost (🔴). Disable them when not in use or switch to lighter alternatives.",
        ...
        "tip_conflict_specific": "{names} all provide {category}. Pick one and remove the rest to recover context budget.",
    ...
    "ko": {
        ...
        "tip_context_efficiency_specific": "{names}이(가) 고비용(🔴) 도구입니다. 사용하지 않을 때 비활성화하거나 가벼운 대안을 검토하세요.",
        "tip_coverage": "필수 카테고리 도구가 부족합니다. git, QA, planning 도구를 추가하세요.",
        "tip_conflict": "기능이 겹치는 도구는 컨텍스트를 낭비합니다. 카테고리별로 하나씩만 유지하세요.",
        "tip_conflict_specific": "{names}이(가) 모두 {category} 기능을 제공합니다. 하나만 남기고 나머지를 제거해 컨텍스트를 확보하세요.",
    ...
    "zh": {
        ...
        "tip_context_efficiency_specific": "{names} 是高成本(🔴)工具。不使用时请禁用，或考虑更轻量的替代方案。",
        "tip_coverage": "配置文件缺少关键类别工具。请添加 git、QA 或规划类工具。",
        "tip_conflict": "功能重叠的工具会浪费上下文。每个类别只保留一个工具。",
        "tip_conflict_specific": "{names} 均提供 {category} 功能。建议只保留一个，移除其余以释放上下文预算。",
